...
address 0x2183e1e73c81b2246c1e2ad7c8b899719a76e08cae4f4df843b882a33b550f7f {
    ...
    module NFTMarketplace {
        ...
        // TODO# 21: Tip NFT Owner
        public entry fun tip_owner(account: &signer, marketplace_addr: address, nft_id: u64, amount: u64) acquires Marketplace {
            let marketplace = borrow_global_mut<Marketplace>(marketplace_addr);
            let nft_ref = vector::borrow_mut(&mut marketplace.nfts, nft_id);

            assert!(nft_ref.owner != signer::address_of(account), 500); // Owner cannot tip themselves
            assert!(amount > 0, 501); // Invalid tip amount

            coin::transfer<aptos_coin::AptosCoin>(account, nft_ref.owner, amount);
        }
